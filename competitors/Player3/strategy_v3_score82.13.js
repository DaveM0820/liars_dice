...
// Strategy: Advanced opponent profiling + Bayesian beliefs + exact probability + multi-dimensional behavior tracking
// Version: 3.0.0
...
onmessage = (e) => {
  ...
  if (momentum >= MOM_HIGH) {
    ...
    raiseThreshold = Math.max(raiseThreshold, 0.45);
  } else if (momentum <= MOM_LOW && currentBid) {
    // Low momentum: be friendlier
    liarThreshold = Math.max(0.18, liarThreshold - 0.04);
  }
  ...
  if (alivePlayers <= 3) {
    ...
    raiseThreshold = Math.max(raiseThreshold, 0.45);
    liarThreshold = Math.max(liarThreshold, 0.20);
  } else if (alivePlayers >= 4) {
    // More players: can be slightly more aggressive early
    if (gameStage === 'early') {
      raiseThreshold = Math.min(raiseThreshold, 0.36);
    }
  }
  ...
  if (claimProb >= 0.30) {
    // Use the best available raise option even if below threshold
    const bestOption = raiseOptions[0];
    // But if momentum is very high, be more cautious
    if (momentum >= MOM_HIGH && probabilityAtLeast(bestOption.f, bestOption.q) < 0.25) {
