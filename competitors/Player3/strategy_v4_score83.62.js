...
// Strategy: Advanced opponent profiling + Bayesian beliefs + exact probability + multi-dimensional behavior tracking + momentum detection + EV-based decisions
// Version: 3.2.0
...
onmessage = (e) => {
  ...
  if (!currentBid) {
    ...
    // Consider all faces, not just the one we have most of
    // Evaluate each face based on expected value
    let bestFace = 1;
    let bestScore = -1;
    ...
    for (let f = 1; f <= 6; f++) {
      ...
      // Score based on probability and quantity (higher is better)
      const prob = probabilityAtLeast(f, testQty);
      const score = prob * testQty; // Favor higher quantity with good probability
  ...
  raiseOptions.forEach(opt => {
    ...
    // Score based on probability and "safety" - prefer higher probability
    // Also consider: higher face at same quantity is slightly safer
    opt.score = opt.prob * 100 + (opt.f > currF ? 5 : 0);
  });
  ...
  if (bestOption.prob >= raiseThreshold * 0.75 && claimProb >= 0.30 && !isLeading) {
    // Close enough and we're not leading - can take calculated risk
  ...
  if (claimProb >= 0.28) {
    ...
    if (bestProb >= 0.28 && claimProb >= 0.32) {
      ...
      shouldRaise = true;
    } else if (bestProb >= 0.22 && claimProb >= 0.30 && riskCount <= 1) {
      // Moderate probabilities, low risk - raise
      shouldRaise = true;
    ...
    if (isLeading && gameStage === 'late' && bestProb < 0.30) {
      shouldRaise = false;
    }
