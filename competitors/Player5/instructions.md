# 🎲 Player 5 - Game-Theoretic Equilibrium Strategy

## Welcome, Competitor!

You are **Player 5**, assigned the **Game-Theoretic Equilibrium Strategy**. Your goal is simple: **WIN**. This is not just a game—it's a competition where only the strongest survive.
...
2. Select **Player5** + 4 opponents (e.g., 4× Baseline)
...
- `Player5/highscores.json` - Your personal high score tracking
- `Player5/strategy_v*.js` - Backup copies of successful versions

**You don't need to copy anything - it all stays in your Player5 directory!**
...
└── Player5/                     # YOU ARE HERE - Equilibrium Strategy
...
## 💡 Your Strategy: Game-Theoretic Equilibrium

### Core Philosophy
You use **equilibrium analysis** to approximate optimal play. Your strategy is based on game theory principles, aiming to minimize exploitability and play near-perfect Liar's Dice.

- **Optimal play**: Minimize exploitability
- **Balanced strategy**: Mix of bluffing and truth-telling
- **Precomputed policy**: Learned from millions of training rounds

### How It Works (Currently)
1. **State representation**: Create an info-set key from your dice, current bid, and player dice counts
2. **Policy lookup**: Check if you have a precomputed equilibrium policy for this state
3. **Fallback**: Currently uses a baseline probability strategy (THIS IS YOUR BIGGEST OPPORTUNITY!)
4. **Action selection**: Choose the highest-probability action from the policy

### Key Strengths (Potential)
- Theoretically strongest approach
- Hard to exploit
- Adapts implicitly through learned policy
- Based on game theory research

### 🚨 CRITICAL OPPORTUNITY
**Your current implementation uses a baseline fallback!**

The biggest improvement you can make:
- **Build/load an equilibrium policy table**: This is the core of your strategy
- **Implement CFR or NFSP**: Or use a precomputed policy
- **Create state-action mappings**: Map game states to optimal actions
- **Use your dice for pseudo-randomness**: Implement mixed strategies deterministically

### Optimization Opportunities
- **CRITICAL**: Build or load an equilibrium policy table (this is your main task!)
- **Improve state representation**: Better encoding of game state
- **Implement policy lookup**: Fast lookup of optimal actions
- **Consider neural network**: Small neural net for policy approximation
- **Use dice for randomization**: Implement mixed strategies using your private dice

### Current Implementation Notes
- Currently uses baseline probability fallback (needs improvement!)
- State key: `${myDiceSorted}|${currentBidStr}|${diceCounts}`
- Policy stored in `self.equilibriumPolicy[infoKey]`
- Falls back to baseline if state not in policy
- **YOU NEED TO BUILD THE POLICY TABLE!**
...
// Strategy: Uses precomputed equilibrium policy
// Version: X.Y.Z
...
2. **Understand** the equilibrium approach (currently uses fallback)
3. **Build/load policy table**: This is your PRIMARY task!
4. **Test** against baseline competitors
5. **Improve** your strategy - Build the policy table!
6. **Test again**
...
**Your Equilibrium strategy is waiting. Your competitors are improving. Every moment counts.**

**⚠️ Remember: You need to build the policy table! This is your biggest opportunity to dominate.**

**Good luck, Player 5. Now go win this tournament.**
