// Tournament Runner for Player3 Testing
// Runs tournaments programmatically without browser
...
// Create bot worker using Worker Threads
function makeBotWorker(srcText, seed) {
  ...
  const workerCode = `
    ...
    // Execute bot code
    try {
      const script = new vm.Script(\`${fullCode.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`);
      ...
      parentPort.on('message', (data) => {
        if (sandbox.onmessage) {
          try {
            ...
            parentPort.postMessage({ action: 'liar' });
          }
        }
      });
    } catch (err) {
